...
impl Validator {
    fn validate(&self, output: &AiOutput, context: &AiContext) -> Result<(), AiDiagnostic> {
        let valid_ids: Vec<u32> = context.atom_index_map.iter().map(|e| e.display_index).collect();
        ...
        for command in &output.result.commands {
            match command {
                Command::PlaceTemplate { template_name, .. } => {
                    if !templates.iter().any(|t| t.name.to_lowercase() == *template_name) {
                        errors.push(format!("Template '{}' not found. Please check the spelling or list available templates.", template_name));
                    }
                }
                Command::AttachFragment { fragment_name, .. } | Command::SubstituteByFragment { fragment_name, .. } => {
                    if !fragments.iter().any(|f| f.name.to_lowercase() == *fragment_name) {
            ...
            let ids = match command {
                ...
                Command::SubstituteByFragment { start_atom_id, end_atom_id, .. } => {
            ...
            if let Some(ids) = ids {
                for id in ids {
                    if !valid_ids.contains(&id) {
                        errors.push(format!("Atom ID {} is invalid. Available IDs are: {:?}", id, valid_ids));
                    }
                }
            }
        }

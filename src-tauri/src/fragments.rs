        ...
        for entry in entries.flatten() {
            if let Ok(content) = fs::read_to_string(entry.path()) {
                if let Ok(file_def) = serde_json::from_str::<FragmentDefinitionFile>(&content) {
                    let template_path = Path::new("templates").join(format!("{}.json", file_def.template_name));
                    if let Ok(template_content) = fs::read_to_string(template_path) {
                        if let Ok(template_json) = serde_json::from_str::<serde_json::Value>(&template_content) {
                            if let Ok(molecule) = serde_json::from_value::<Molecule>(template_json["molecule"].clone()) {
